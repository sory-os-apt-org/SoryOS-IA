...
describe('RemoteMock streams', () => {
  ...
  it('waits for opens with opened(), and answers $events with one ready frame per generation', async () => {
    ...
    await expect(take(first, 1)).resolves.toEqual([{ type: 'ready', clientId: 'mock-client-1', host: { home: '/home/me', executionWorld: 'local' } }])
    const again = mock.open('$events', [{}], idle())
    await expect(second).resolves.toBeUndefined()
    await expect(mock.streams.opened('$events', 1)).resolves.toBeUndefined()
    await expect(take(again, 1)).resolves.toEqual([{ type: 'ready', clientId: 'mock-client-2', host: { home: '/home/me', executionWorld: 'local' } }])
